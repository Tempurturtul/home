...
/**
 * Attempts to get a specific user.
 ...
 */
async function getUser(name, requestingUser) {
	const result = {};

	// Fail if requesting user isn't an admin or the target user.
	...
	return db.oneOrNone('SELECT name, role FROM users WHERE name = $1', [name])
		.then((user) => {
